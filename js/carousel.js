...
Carousel.prototype = {
    ...
    init: function(urlArr) {                //创建dom结构
        ...
        this.wrap.innerHTML = '<span id="' + this.wrapId + '_pre" class="fa fa-angle-left fa-3x"></span><span id="' 
        + this.wrapId + '_next" class="fa fa-angle-right fa-3x"></span><ul id="' + this.wrapId + '_page"></ul><div id="'
        + this.wrapId + '_container"></div>';
        ...
        for (let value of urlArr) {         //构建圆点
            var str=value.substring(5,6);
            container.innerHTML += '<div class="' + this.wrapId + '_img-item"><img src="' + value + '"></div>';
            page.innerHTML += '<li class="' + this.wrapId + '_pagination">'+str+'</li>';
        }
